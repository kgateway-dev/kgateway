...
type IstioIntegration struct {
	LabelNewNamespace    bool `json:"labelNewNamespace,omitempty" desc:"If creating a namespace for Gloo, include the 'istio-injection: enabled' label to allow Istio sidecar injection for most Gloo pods. Keep in mind that Istio's default behavior' will auto-inject a sidecar into all pods in such a marked namespace. Disabling this behavior in Istio or using the global.istioIntegration.disableAutoinjection flag is recommended."`
	WhitelistDiscovery   bool `json:"whitelistDiscovery,omitempty" desc:"Mark the discovery pod for istio sidecar injection."`
	DisableAutoinjection bool `json:"disableAutoinjection,omitempty" desc:"Mark all pods (excluding those whitelisted by other config values) to avoid injecting Istio sidecars when Istio's auto-injection is enabled. It's recommended that this be set to false if Gloo's namespace is marked for Istio discovery, as most pods don't play well with injection with extra manual configuration."`
}
