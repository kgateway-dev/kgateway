	...
	var allTests = func(rendererTestCase renderTestCase) {
		...
		When("kube gateway integration is enabled", func() {
			...
			It("renders default GatewayParameters", func() {
				...
				Expect(gwpKube.GetIstio().GetIstioContainer().GetIstioDiscoveryAddress().GetValue()).To(Equal("istiod.istio-system.svc:15012"))
				Expect(gwpKube.GetIstio().GetIstioContainer().GetIstioMetaMeshId().GetValue()).To(Equal("cluster.local"))
				Expect(gwpKube.GetIstio().GetIstioContainer().GetIstioMetaClusterId().GetValue()).To(Equal("Kubernetes"))
