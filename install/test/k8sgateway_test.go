...
var _ = Describe("Kubernetes Gateway API integration", func() {
	allTests := func(rendererTestCase renderTestCase) {
		...
		prepareHelmManifest := func(namespace string, values glooeetestutils.HelmValues) {
		...
		JustBeforeEach(func() {
			prepareHelmManifest(namespace, glooeetestutils.HelmValues{ValuesArgs: valuesArgs})
		})
