...
get_resources_cmd = "{0} -n {1} template {2} --include-crds charts/kgateway/ --set image.pullPolicy='Never' --set image.registry=ghcr.io/kgateway-dev --set image.tag='{3}'".format(helm_cmd, settings.get("helm_installation_namespace"), settings.get("helm_installation_name"), image_tag)
...
def install_kgateway():
    if not kgateway_installed :
        install_helm_cmd = """
            ...
            {0} upgrade --install -n {1} --create-namespace kgateway-crds charts/kgateway-crds ;
            {0} upgrade --install -n {1} --create-namespace {2} charts/kgateway/ --set controller.image.pullPolicy='Never' --set image.registry=ghcr.io/kgateway-dev --set image.tag='{3}'""".format(helm_cmd, settings.get("helm_installation_namespace"), settings.get("helm_installation_name"), image_tag)
        for f in settings.get("helm_values_files") :
