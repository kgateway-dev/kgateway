	...
	Context("Benchmark Gloo Translation", func() {
		...
		Measure("it should perform translation efficiently", func(b Benchmarker) {
			...
			b.Time(fmt.Sprintf("runtime of fnv hash translate"), func() {
				snap, errs, report, err := fnvTranslator.Translate(params, proxyClone, []*v1.Upstream{})
				Expect(err).NotTo(HaveOccurred())
				Expect(errs.Validate()).NotTo(HaveOccurred())
				Expect(snap).NotTo(BeNil())
				Expect(report.GetProxyReport()).To(matchers2.BeEquivalentToDiff(validationutils.MakeReport(proxy)))
			})
			b.Time(fmt.Sprintf("runtime of hashstructure translate"), func() {
				snap, errs, report, err := hashstructureTranslator.Translate(params, proxyClone, []*v1.Upstream{})
				Expect(err).NotTo(HaveOccurred())
				Expect(errs.Validate()).NotTo(HaveOccurred())
				Expect(snap).NotTo(BeNil())
				Expect(report.GetProxyReport()).To(matchers2.BeEquivalentToDiff(validationutils.MakeReport(proxy)))
