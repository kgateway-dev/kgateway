...
func (s *testingSuite) TestPersistInvalidVirtualService() {
	...
	helpers.EventuallyResourceRejected(func() (resources.InputResource, error) {
		return s.testInstallation.ResourceClients.VirtualServiceClient().Read(s.testInstallation.Metadata.InstallNamespace, validation.InvalidVsName, clients.ReadOpts{
			Ctx: s.ctx,
		})
	})
	s.testInstallation.Assertions.AssertEventualCurlResponse(
...
func (s *testingSuite) TestMissingUpstream() {
	...
	helpers.EventuallyResourceWarning(func() (resources.InputResource, error) {
		return s.testInstallation.ResourceClients.VirtualServiceClient().Read(s.testInstallation.Metadata.InstallNamespace, validation.ValidVsName, clients.ReadOpts{})
	})
