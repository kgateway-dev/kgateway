...
// +kubebuilder:validation:Enum=DisableAll;LastWins
...
const (
	...
	// ExtAuthLastWins allows the filter to be enabled based on the Extension if present.
	ExtAuthLastWins ExtAuthEnabled = "LastWins"
)
...
type ExtAuthRoutePolicy struct {
	// Extension references the ExternalExtension that should be used for authentication.
	// +optional
	...
	// When set to "LastWins", the filter is enabled if an extension is provided.
	// When empty, the filter is enabled if an extension is provided.
	// +optional
...
spec:
  ...
  - backendRef:
    - name: custom-auth.svc
      port: 50051
  placement: # note that this is just a proposal and is likely to not be implemented in first pass
    stage: RateLimit 
    priority: 1
---
...
spec:
  ...
  extauth:
    enablement: 
      type: DisableAll  # note we propose just having this option to begin with so if its present you can enable health check routes easily
---
