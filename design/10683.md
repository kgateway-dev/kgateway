...
const (
	...
	// ExtAuthLastWins allows the filter to be enabled based on the extensionRef if present.
...
type ExtAuthRoutePolicy struct {
	...
	// When set to "LastWins", the filter is enabled if an extensionRef is provided.
	// When empty, the filter is enabled if an extensionRef is provided.
	// +optional
