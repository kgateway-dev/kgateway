...
### API Changes
Requires extension policy [implementation](https://github.com/kgateway-dev/kgateway/issues/10851).
```yaml
...
### Plugin
For this initial pass we should implement ExtendedPolicy and the basic reference on routepolicy. We may want to not implement route level disablement in the first pass.
Given the concerns for extensibility that we have some type of generalized ExtendedPolicy type for external destinations such as extauth and tracing.
