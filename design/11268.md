...
For `Gateways`, a policy targeting a `Listener` is higher priority than a policy targeting the whole `Gateway`. `PListener-t0 > ... PListner-tn > PGateway-t0 >  ... PGateway-tn`, where PListener is a policy targeting a `Listener` using `sectionName`, PGateway is a policy targeting the `Gateway`, and `t` is the creation timestamp.
