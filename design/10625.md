...
Default is `OFF` for backward compatibility. Expect to switch the chart default to `BASIC` in the next minor release and encourage users to opt in to `STRICT` for strict mode for better safety guarantees.
