...
### Alternative 2: The "LambdaConfig" config is embedded in the Upstream API

This alternative is related to the first alternative, but it is focused on embedding the "LambdaConfig" config directly in the Upstream API. This config will be shared by the Upstream and RoutePolicy APIs. For users that want to interact with a single API, and don't need composability, this may be a simpler solution. For users that want to use the Upstream API as a composable entity, then the UX will be the exact same as the proposed solution. Ultimately, this alternative is more flexible than the proposed solution, but it requires more work to implement and may have UX impliciations as it introduces multiple entrypoints for lambda configuration.

### Alternative 3: Use a synthetic parameter in backendRef filters
...
### Alternative 4: (Implementation Detail) Use envoy-gloo's lambda filter
