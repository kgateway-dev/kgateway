...
- Companion: `13586-referenced-only-cluster-discovery.md` specifies the same feature on top of the #14184 publication engine (plus #14343 and #14257), which is the smaller change if that stack lands. This variant exists because none of that machinery is on `main`: `714190cf85` reverted the earlier per-client readiness gates in favor of a first-connect grace period, so a design that assumes them cannot ship today. The two share an irreducible core - the same over-approximating emittable set, the same unresolvable-reference guard, the same golden-corpus completeness property test, and the same de-reference grace. They differ only in whether the addition-side hold is written here or grafted onto the engine's existing bounded hold.
...
- Resurrect the traversal scaffolding from `714190cf85`, replace the two-type extraction with all-string collection, and add request-time-selector detection over all three oneof arms plus the plugin-facing claim API (explicit names and the `{namespace, port}` predicate). Detection and claims ship with the collector, so the filter can never be enabled without them.
...
The companion EP (`13586-referenced-only-cluster-discovery.md`) takes exactly this dependency, on #14343, #14257, and the #14184 engine PR landing first. That is the better change *if* the stack lands: it needs no new publication machinery, gets EDS alignment for free, and shrinks per-client control-plane memory too. But none of it is on `main` today, and #13868 and #13958, which supplied the referenced-set walk, were reverted outright by `714190cf85`. Making a user-visible fix wait on three unmerged PRs, two marked `[WIP]` and one still on a fork, whose predecessor was already reverted once, is a scheduling risk this variant exists to avoid: everything it needs is either merged or small enough to specify here.
...
- **Whether to ship Option B, the Service label selector, first** as an immediate mitigation for the reporter while this lands. Request-time destinations strengthen the case independently: operator-declared inclusion composes with unenumerable destination sets, which derived pruning cannot see at all.
