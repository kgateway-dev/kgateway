...
Note: `SecretSelector` is an existing type that is defined in `api/v1alpha1/shared_types.go` and is shared between agentgateway and traffic policies.
...
```json
{
  ...
  "typed_config": {
    ...
    "forwarding": {
      "header": "x-client-id",  // Configurable via clientIdHeader field
...
3. **Filter Configuration:**
   ...
   - Set `forwarding.header` based on `clientIdHeader` (defaults to "x-client-id")
...
- Test clientIdHeader configuration (default "x-client-id" and custom values)
