...
func filterEndpointResourcesForStaticClusters(clusters envoycache.Resources, endpoints envoycache.Resources) envoycache.Resources {
	...
	for _, item := range endpoints.Items {
		...
		if _, isStatic := staticNames[cla.GetClusterName()]; isStatic {
			continue
		}
		filtered = append(filtered, item)
	}
	if len(filtered) == len(endpoints.Items) {
		return endpoints
	}
	return envoycache.NewResourcesWithTTL(endpoints.Version, filtered)
}
