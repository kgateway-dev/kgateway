...
func parseTLSParameters(tlsParameters *kgateway.TLSParameters) (*envoytlsv3.TlsParameters, error) {
	...
	ciphers, err := sslutils.ValidateTlsStrings(tlsParameters.CipherSuites, sslutils.SupportedCipherSuites, "cipher suite")
	if err != nil {
		return nil, err
	}

	curves, err := sslutils.ValidateTlsStrings(tlsParameters.EcdhCurves, sslutils.SupportedEcdhCurves, "ECDH curve")
	if err != nil {
		return nil, err
	}
