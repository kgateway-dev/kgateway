...
var _ validator.Validator = &capturingValidator{}

func (c *capturingValidator) Validate(_ context.Context, b *envoybootstrapv3.Bootstrap) error {
	c.bootstrap = b
	return nil
}
...
func TestValidateXDSIncludesRateLimitQuotaFilter(t *testing.T) {
	settings, err := buildRateLimitQuotaBucketSettings(&kgateway.RateLimitQuotaPolicy{Bucket: []kgateway.RateLimitQuotaBucketEntry{{
		Key: "service", Type: kgateway.RateLimitQuotaBucketEntryTypeGeneric, Value: new("api"),
	}}})
	require.NoError(t, err)

	policy := &TrafficPolicy{spec: trafficPolicySpecIr{rateLimitQuota: &rateLimitQuotaIR{
		provider:       testRateLimitQuotaProvider("default/rlqs"),
		bucketSettings: settings,
	}}}
	v := &capturingValidator{}
	require.NoError(t, validateXDS(t.Context(), policy, v, false))
	require.NotNil(t, v.bootstrap)

	var names []string
	for _, l := range v.bootstrap.GetStaticResources().GetListeners() {
		for _, fc := range l.GetFilterChains() {
			for _, f := range fc.GetFilters() {
				...
				for _, hf := range hcm.GetHttpFilters() {
					names = append(names, hf.GetName())
				}
			}
		}
	}
	assert.Contains(t, names, "ratelimit/quota/default/rlqs",
		"strict validation must exercise the assembled RLQS listener filter")
}
