...
type requestRedirectIr struct {
	// Redir is the base redirect action (hostname, status code, scheme rewrite, path
	// rewrite). PortRedirect is intentionally left unset here and is resolved per
	// listener in applyRedirectPortPostProcessing, because a single HTTPRoute may
	// attach to multiple Gateway listeners on different ports.
	Redir *envoyroutev3.RedirectAction
	// Scheme and Port preserve the user's raw intent from the HTTPRequestRedirectFilter
	// so we can resolve the effective redirect port per listener without losing the
	// distinction between "user omitted this field" and "user picked a default value".
	Scheme *string
...
func (r *requestRedirectIr) apply(
	...
	// Clone so per-listener PortRedirect resolution in applyRedirectPortPostProcessing
	// doesn't mutate a proto that is shared across listeners via the PolicyIR.
	outputRoute.Action = &envoyroutev3.Route_Redirect{
		Redirect: proto.Clone(r.Redir).(*envoyroutev3.RedirectAction),
	}
}
...
//   - Else if filter.Scheme is set, use the scheme's well-known port (http=80,
//     https=443).
//   - Else, use the Gateway Listener port.
//
// Per the spec, the port is omitted from the Location header when the effective
// port matches the scheme's well-known default:
//
//   - http + 80
//   - https + 443
//
...
func applyRedirectPortPostProcessing(
	...
	// Determine the effective scheme. If the filter explicitly sets a scheme, use
	// that; otherwise infer from the listener's protocol.
	...
	// Omit the port from the Location header when it matches the scheme's
	// well-known default.
