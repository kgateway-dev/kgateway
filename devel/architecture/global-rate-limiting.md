...
kgateway integrates with any service that implements the Envoy Rate Limit gRPC protocol. For your convenience, we provide an example deployment using the official Envoy rate limit service in the [test/e2e/features/rate_limit/testdata](../test/e2e/features/rate_limit/testdata) directory.

```bash
kubectl apply -f test/e2e/features/rate_limit/testdata/rate-limit-server.yaml
```
