...
var _ = Describe("Validator", func() {
	...
	Context("validating gloo resources", func() {
		...
		Context("artifacts", func() {
			It("accepts an artifact deletion when validation succeeds", func() {
				vc.validate = acceptProxy
				...
				ref := artifact.GetMetadata().Ref()
				err = v.ValidateDeleteArtifact(context.TODO(), ref, false)
				Expect(err).NotTo(HaveOccurred())
			})
			It("rejects an artifact deletion when validation fails", func() {
				vc.validate = failProxy
			...
			It("accepts an artifact deletion when there is a validation warning and allowWarnings is true", func() {
				vc.validate = warnProxy
				v.allowWarnings = true
			...
			It("rejects an artifact deletion when there is a validation warning and allowWarnings is false", func() {
				vc.validate = warnProxy
				v.allowWarnings = false
