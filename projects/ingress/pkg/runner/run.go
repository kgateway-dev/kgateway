...
var _ bootstrap.Runner = new(ingressRunner)
...
func NewIngressRunner() *ingressRunner {
	return &ingressRunner{}
}

func (i ingressRunner) Run(ctx context.Context, kubeCache kube.SharedCache, inMemoryCache memory.InMemoryResourceCache, settings *gloov1.Settings) error {
	var (
		cfg           *rest.Config
	...
	proxyFactory, err := gloobootstrap.ConfigFactoryForSettings(params, gloov1.ProxyCrd)
	if err != nil {
		return err
	}

	upstreamFactory, err := gloobootstrap.ConfigFactoryForSettings(params, gloov1.UpstreamCrd)
	if err != nil {
		return err
	}
