...
func CreateCmd(opts *options.Options, optionsFunc ...cliutils.OptionsFunc) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:    "Create a secret. Note that for TLS, OAuth and ApiKey secrets, the annotation resource_kind: '*v1.Secret' is needed in order for Gloo to find that secret.",
