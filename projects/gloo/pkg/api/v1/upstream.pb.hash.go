...
func (m *Upstream) Hash(hasher hash.Hash64) (uint64, error) {
	...
	case *Upstream_Cloudmap:
		...
		if h, ok := interface{}(m.GetCloudmap()).(safe_hasher.SafeHasher); ok {
			if _, err = hasher.Write([]byte("Cloudmap")); err != nil {
				return 0, err
			}
			if _, err = h.Hash(hasher); err != nil {
				return 0, err
			...
			if fieldValue, err := hashstructure.Hash(m.GetCloudmap(), nil); err != nil {
				return 0, err
			} else {
				if _, err = hasher.Write([]byte("Cloudmap")); err != nil {
					return 0, err
