...
// Equal function
func (m *AdaptiveRequestConcurrencyPolicySpec_MinRoundtripTimeCalculationParams) Equal(that interface{}) bool {
	if that == nil {
		return m == nil
	}

	target, ok := that.(*AdaptiveRequestConcurrencyPolicySpec_MinRoundtripTimeCalculationParams)
	if !ok {
		that2, ok := that.(AdaptiveRequestConcurrencyPolicySpec_MinRoundtripTimeCalculationParams)
		if ok {
			target = &that2
		} else {
			return false
