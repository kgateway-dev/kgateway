...
import (
	...
	tunneling "github.com/solo-io/gloo/projects/gloo/pkg/plugins/tunneling"
...
func getClusters(glooSnapshot *v1snap.ApiSnapshot, xdsSnapshot envoycache.Snapshot) map[string]struct{} {
	...
	for clusterName := range xdsClusters.Items {
		if strings.HasPrefix(clusterName, dfp.DfpAutogeneratedClusterPrefix) {
			// dfp clusters are always valid, and should not be censored
			// dynamic forward proxy clusters, in particular, are _never_ in gloo snapshots (the above check)
			// this is because they explicitly do not have a corresponding upstream: https://www.solo.io/blog/dynamic-forward-proxy-with-gloo-edge/
			validClusters[clusterName] = struct{}{}
		} else if strings.HasPrefix(clusterName, tunneling.TunnelingAutogeneratedClusterPrefix) {
			// http tunneling clusters are always valid, and should not be censored
			// (identical rationale to above)
			validClusters[clusterName] = struct{}{}
		}
		// in the future, other prefixes can/should be added here based on whether or not the
		// "exist in gloo snapshot's upstreams" check is not a valid assumption
	}
