...
var _ = Describe("Translate Proxy", func() {
	...
	It("uses listeners and routes from the previous snapshot when sanitization fails", func() {
		...
		oldListeners := oldXdsSnap.GetResources(xds.ListenerTypev3)
		newListeners := xdsCache.setSnap.GetResources(xds.ListenerTypev3)

		Expect(oldListeners).To(Equal(newListeners))

		oldRoutes := oldXdsSnap.GetResources(xds.RouteTypev3)
		newRoutes := xdsCache.setSnap.GetResources(xds.RouteTypev3)
