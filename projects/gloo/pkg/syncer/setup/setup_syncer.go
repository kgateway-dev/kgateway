...
func (s *setupSyncer) Setup(ctx context.Context, kubeCache kube.SharedCache, memCache memory.InMemoryResourceCache, settings *v1.Settings) error {
	...
	if s.validationServer == emptyValidationServer {
		...
		if maxGrpcMsgSize := settings.GetGateway().GetValidation().GetValidationServerGrpcMaxSize(); maxGrpcMsgSize != 0 {
			validationGrpcServerOpts = append(validationGrpcServerOpts, grpc.MaxRecvMsgSize(int(maxGrpcMsgSize)))
		}
