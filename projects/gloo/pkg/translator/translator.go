...
func (t *translatorInstance) Translate(
	...
	upstreamRefToEndpoints := createUpstreamToEndpointsMap(params.Snapshot.Upstreams, params.Snapshot.Endpoints)
	...
	clusters := t.computeClusters(params, reports, upstreamRefToEndpoints, proxy)
	logger.Debugf("computing envoy endpoints for proxy: %v", proxy.Metadata.Name)

	endpoints := t.computeClusterEndpoints(params, upstreamRefToEndpoints, reports)
