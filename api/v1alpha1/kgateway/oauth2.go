...
type OAuth2Provider struct {
	...
	// Discovery is performed by the control plane rather than the proxy, and takes its trust anchors from the CA
	// certificates of the BackendTLSPolicy attached to backendRef, so an issuer served under a private or self-signed
	// CA needs no additional trust configuration on the control plane. The issuer URI's own host is authenticated,
	// since the control plane connects to it directly rather than through the proxy's cluster.
	// +optional
	//
