...
type OAuth2Provider struct {
	...
	// Discovery is performed by the control plane rather than the proxy, and validates the issuer using the
	// BackendTLSPolicy or BackendConfigPolicy attached to backendRef: its CA certificates supply the trust anchors and
	// its hostname is the identity verified. An issuer served under a private or self-signed CA therefore requires no
	// additional trust configuration on the control plane beyond that policy.
	// +optional
	//
