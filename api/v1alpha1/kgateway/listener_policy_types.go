...
type HTTPSettings struct {
	...
	// LocalReplyConfig customizes the responses that Envoy itself generates (local replies),
	// for example to rewrite the status code or body of error responses produced by the proxy
	// (such as 503s when no healthy upstream is available) rather than responses proxied from a backend.
	// See here for more information: https://www.envoyproxy.io/docs/envoy/latest/configuration/http/http_conn_man/local_reply
	// +optional
	LocalReplyConfig *LocalReplyConfig `json:"localReplyConfig,omitempty"`
}

// LocalReplyConfig customizes the local replies (responses generated by Envoy itself) for a listener.
// See here for more information: https://www.envoyproxy.io/docs/envoy/latest/api-v3/extensions/filters/network/http_connection_manager/v3/http_connection_manager.proto#envoy-v3-api-msg-extensions-filters-network-http-connection-manager-v3-localreplyconfig
type LocalReplyConfig struct {
	// Mappers configures how matching local replies are rewritten. Mappers are evaluated in
	// order and the first mapper whose statusCodeMatch matches the local reply is applied;
	// subsequent mappers are ignored. A local reply that does not match any mapper is returned unchanged.
	// +kubebuilder:validation:MinItems=1
	// +kubebuilder:validation:MaxItems=16
	// +required
	Mappers []ResponseMapper `json:"mappers"`
}

// ResponseMapper rewrites a local reply whose StatusCodeMatch matches the response code.
// See here for more information: https://www.envoyproxy.io/docs/envoy/latest/api-v3/extensions/filters/network/http_connection_manager/v3/http_connection_manager.proto#envoy-v3-api-msg-extensions-filters-network-http-connection-manager-v3-responsemapper
type ResponseMapper struct {
	// StatusCodeMatch selects which local replies this mapper applies to, by comparing against
	// the response status code (e.g. op=GE value=500 matches all 5xx responses).
	// +required
	StatusCodeMatch StatusCodeMatcher `json:"statusCodeMatch"`

	// StatusCode overrides the HTTP status code returned to the client for matching local replies.
	// If unset, the original status code is preserved.
	// +optional
	// +kubebuilder:validation:Minimum=200
	// +kubebuilder:validation:Maximum=599
	StatusCode *int32 `json:"statusCode,omitempty"`

	// Body specifies a custom, static response body (e.g. an HTML or JSON document), provided as
	// an inline string, for matching local replies. The body is returned verbatim. If unset, the
	...
	// ContentType overrides the Content-Type header returned with the local reply
	// (e.g. "text/html; charset=utf-8" or "application/json"). If unset, Envoy returns the body
	// with its default Content-Type (text/plain).
	// +optional
	// +kubebuilder:validation:MinLength=1
...
type StatusCodeMatcher struct {
	// Op is the comparison operator (EQ, GE or LE) applied between the response status code and Value.
	// +required
	Op Op `json:"op"`

	// Value is the HTTP status code to compare against.
	// +required
