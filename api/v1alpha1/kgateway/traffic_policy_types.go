...
type LocalRateLimitPolicy struct {
	...
	// Descriptors defines per-descriptor rate limits that apply in addition to the shared token bucket.
	// Each descriptor specifies key-value entries and a token bucket applied to requests whose
	// generated descriptor entries match. Rate limit actions are automatically generated for each
	// descriptor using generic key actions.
	// +optional
...
// LocalRateLimitDescriptor defines a descriptor for local rate limiting.
// When a request's generated descriptor entries match all entries in this descriptor,
// the specified token bucket is applied to rate limit that request.
type LocalRateLimitDescriptor struct {
	// Entries are the key-value pairs that must match the generated descriptor entries
	// for this descriptor's token bucket to be applied.
	// All entries must match for the descriptor to be considered a match.
	// +required
	// +kubebuilder:validation:MinItems=1
	Entries []LocalRateLimitDescriptorEntry `json:"entries"`

	// TokenBucket defines the rate limit to apply when this descriptor matches.
	// +required
	TokenBucket TokenBucket `json:"tokenBucket"`
}

// LocalRateLimitDescriptorEntry defines a key-value entry for local rate limit descriptor matching.
// Entries are matched against descriptor entries generated by rate limit actions on the route.
type LocalRateLimitDescriptorEntry struct {
	// Key is the descriptor key.
	// +required
	// +kubebuilder:validation:MinLength=1
	Key string `json:"key"`

	// Value is the descriptor value to match against.
	// +required
	// +kubebuilder:validation:MinLength=1
	Value string `json:"value"`
}
