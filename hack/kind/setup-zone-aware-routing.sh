...
# The region and zones assigned to the worker nodes. The zone-aware routing
# e2e test expects these values.
ZONE_REGION="us-east-1"
ZONES=(us-east-1a us-east-1b us-east-1c)

cd "${ROOT_DIR}"

# Mirrors create_kind_cluster_or_skip in setup-kind.sh, with a multi-node
# config: zone-aware routing needs one worker node per zone.
...
# Reuse the standard development workflow for everything that is not
# zone-aware specific: Gateway API CRDs, MetalLB, building and loading the
# images, packaging the charts, and deploying kgateway.
