...
| `autoHostRewrite` | [.google.protobuf.BoolValue](https://developers.google.com/protocol-buffers/docs/reference/csharp/class/google/protobuf/well-known-types/bool-value) | Enable \ Disable auto host re-write. If not set, the default behavior is to enable auto host rewrite, if the first host in the list is not an IP address (i.e. it is a DNS adress) If enabled (explicitly or by the default) auto_host_rewrite will be automatically added to routes that point to this upstream. |  |
