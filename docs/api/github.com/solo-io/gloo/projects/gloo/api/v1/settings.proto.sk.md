...
"ratelimit": .ratelimit.plugins.gloo.solo.io.EnvoySettings
...
| `ratelimit` | [.ratelimit.plugins.gloo.solo.io.EnvoySettings](../enterprise/plugins/ratelimit/ratelimit.proto.sk#envoysettings) | Enterprise-only: Partial config for GlooE's rate-limiting service, based on Envoy's rate-limit service; supports Envoy's rate-limit service API. (reference here: https://github.com/lyft/ratelimit#configuration) Configure rate-limit *descriptors* here, which define the limits for requests based on their descriptors. Configure rate-limit *actions*, which define how request characteristics get translated into descriptors, on the VirtualHost or its routes |  |
