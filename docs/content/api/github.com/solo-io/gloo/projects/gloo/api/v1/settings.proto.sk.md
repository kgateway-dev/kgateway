...
"proxyGarbageCollection": .google.protobuf.BoolValue
...
| `proxyGarbageCollection` | [.google.protobuf.BoolValue](https://developers.google.com/protocol-buffers/docs/reference/csharp/class/google/protobuf/well-known-types/bool-value) | Set this option to determine the state of the envoy configuration when a virtual service is deleted, resulting in a proxy with no configured routes. set to true if you wish to reset the envoy configuration to a clean slate with no routes. set to false if you wish to keep envoy serving the routes from the latest valid configuration. If not specified, defaults to `false`. |  |
