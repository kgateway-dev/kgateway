...
2. The new PEM-encoded CA certificate is added to the `gloo-mtls-certs` secret alongside the old CA certificate that is routated out.
...
4. The new PEM-encoded server certificate and private key are added to the secret.
5. Gloo Edge waits for the duration that is set in `gateway.certGenJob.rotationDuration` before continuing to the next step.
6. The old TLS credentials are removed from the `gloo-mtls-certs` secret. All workloads now use the new TLS credentials.
