---
title: "Integrating Gloo Gateway and Let's Encrypt with cert-manager"
menuTitle: Cert-manager
description: Secure your ingress traffic using Gloo Gateway and cert-manager
weight: 20
---

This document shows how to secure your traffic using Gloo Gateway, Let's Encrypt, and cert-manager. This guide assumes you already 
have a Kubernetes cluster up and running. Further, it assumes your cluster has a load-balancer such that when 
Gloo Gateway is installed, the proxy service is granted an external IP. This guide will show examples for both the DNS-01 and HTTP-01 challenges.
...
gateway proxy that we installed with Gloo Gateway.
...
Create the certificate for the Gloo Gateway ingress:
```shell
cat << EOF | kubectl apply -f -
apiVersion: cert-manager.io/v1
kind: Certificate
...
### Expose the service securly via Gloo Gateway

Configure Gloo Gateway's default Virtual Service to route to the function and use the certificates.
...
{{% notice note %}}
These steps are specific for Gloo Gateway running in gateway mode. When running in ingress mode, since cert-manager will automatically create `Ingress` resources, you will not need to add/modify `VirtualService` resources.
{{% /notice %}}
...
Notice the use of the `http01` solver. By default, cert-manager will create a `Service` of type `NodePort` to be routed via an `Ingress`. However, since we are running Gloo Gateway in gateway mode, incoming traffic is routed via a `VirtualService` and does not require a `NodePort`, so we are explicitly setting the `serviceType` to `ClusterIP`. 
...
Now that the pod which will serve this token is created, we need to configure Gloo Gateway to route to it. In this case, we will create a Virtual Service for our custom domain that will route requests for the path `/.well-known/acme-challenge/<TOKEN>` to the cert-manager created pod.
