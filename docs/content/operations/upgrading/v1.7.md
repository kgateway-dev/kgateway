...
weight: 60
description: Upgrading to Gloo Gateway 1.7.x and Gloo Gateway Enterprise 1.7.x
---

In this guide we will describe the necessary steps to upgrade your Gloo Gateway or Gloo Gateway Enterprise deployments to their `1.7`
versions using [Helm](https://github.com/helm/helm). This guide also includes upgrade steps to Gloo Gateway v1.6, as
that is required as an intermediate step for upgrading to v1.7 from earlier versions without downtime. This is because Gloo Gateway v1.6
supports both v2 and v3 of Envoy's API in preparation of Envoy's plan to drop support for V2 in 1.18 (i.e., Gloo 1.7).
Gloo Gateway v1.7 drops support for Envoy's v2 API, so upgrading Gloo Gateway to v1.7 requires a pit stop upgrade at v1.6 to 
transition from Envoy v2 to v3 and maintain a continuous uptime.
 
The guide assumes that you are running Gloo Gateway 1.5.0+ or Gloo Gateway Enterprise 1.5.0+. 
...
Install Gloo Gateway or Gloo Gateway Enterprise v1.5.
...
#### Upgrading the Gloo Gateway deployment to v1.6
...
```
{{% /tab %}}
{{% tab name="Gloo Gateway with downtime check" %}}
```shell script
...
```
{{% /tab %}}
{{% tab name="Gloo Gateway Enterprise with downtime check" %}}
```shell script
...
Gloo Gateway's default helm configurations for v1.6 and v1.7 will smoothly transistion from v2 to v3. However, if you have
...
#### Upgrading the Gloo Gateway deployment to v1.7
