...
* **Leader election**: Now, the Gloo Edge control plane deployment supports leader election for multiple replicas of the `gloo` deployment. This way, the `gloo` control plane is more highly available by default. If you previously used xDS relay, you can uninstall that component, as the Gloo Edge xDS server is now highly available by default. If you already run multiple instance of Gloo Edge for high availability, make sure that leader election is enabled by setting the `gloo.disableLeaderElection` value to `false`.
