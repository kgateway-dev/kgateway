...
weight: 38
description: Upgrading to Gloo Gateway 1.13.x and Gloo Gateway Enterprise 1.13.x
---

Review and apply version-specific changes in Gloo Gateway 1.13.x and Gloo Gateway Enterprise 1.13.x before upgrading your Gloo Gateway installation.
...
Review the following highlights of features changes in Gloo Gateway version 1.13.
...
* **New! Canary upgrades for Gloo Gateway Federation**: Now, you can upgrade your Gloo Gateway Federation installation in a canary model. For more information, see [Canary Upgrade]({{% versioned_link_path fromRoot="/operations/upgrading/canary/" %}}).

* **Improved config validation**: By default, Gloo Gateway checks the configuration of Gloo custom resources to prevent invalid config. Now, configuration validation is also automatically checked for add-ons such as rate limiting and external auth custom resources. For more information, see [Config Reporting & Validation]({{% versioned_link_path fromRoot="/guides/traffic_management/configuration_validation/" %}}).

* **Leader election**: Now, the Gloo Gateway control plane deployment supports leader election for multiple replicas of the `gloo` deployment. This way, the `gloo` control plane and its components, such as the Gloo Gateway xDS server, are more highly available by default. During the installation, one of the gloo pods is elected leader. If the elected leader pod fails due to networking, configuration, or other issues, a new leader is elected among the remaining `gloo` pods. If you previously used xDS relay to provide high availability for the xDS server, you can uninstall that component and use the built-in replica support for the `gloo` deployment. To run only one replica of `gloo`, you can disable leader election by setting the `gloo.disableLeaderElection` value to `true`.

**Deprecated features**:
* **xDS relay**: With the introduction of leader election for the Gloo Gateway control plane, xDS relay is no longer needed to increase the availability of the xDS server. Previously, the xDS relay was a beta feature in Gloo Gateway 1.11 or 1.12, and not available for non-default installation modes of Gloo Gateway, such as REST Endpoint Discovery (EDS), mTLS, or Istio mTLS modes.
...
* When you run [multiple instances of Gloo Gateway]({{% versioned_link_path fromRoot="/installation/advanced_configuration/multiple-gloo-installs/" %}}), you must set the `gloo.disableLeaderElection` value to `false`.
...
**Deprecated CRDs**:
* The `IgnoreProxyValidationFailure` flag is deprecated because the Gloo Gateway validation webhook now validates rate limit configuration by default.
