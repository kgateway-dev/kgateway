...
* **Validate Kubernetes secrets on removal**: Fixed an issue in the validation webhook where Kubernetes secrets were not validated on deletions. You can opt our of secret validation by using the `gateway.validation.webhook.skipDeleteValidationResources` Helm value. 
