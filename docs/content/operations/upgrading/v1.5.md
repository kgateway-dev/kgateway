...
weight: 65
description: Upgrading to Gloo Gateway 1.5.x and Gloo Gateway Enterprise 1.5.x
---

In this guide we will describe the necessary steps to upgrade your Gloo Gateway or Gloo Gateway Enterprise deployments to their `1.5` 
versions using [Helm](https://github.com/helm/helm)
 
The guide assumes that you are running Gloo Gateway 1.3.0+ or Gloo Gateway Enterprise 1.3.0+. 
...
In Gloo Gateway 1.5.0-beta8 and Gloo Gateway Enterprise 1.5.0-beta3 we introduced a new 
...
The second option involves using in the template that is shipped in the Gloo Gateway and Gloo Gateway enterprise charts.
...
```
{{% /tab %}}
{{% tab name="Gloo Gateway Enterprise - Helm 2" %}}
```shell script
helm repo update
...
#### Upgrading the Gloo Gateway deployment
Now that we have installed the new CRD to cluster, you can upgrade your Gloo Gateway deployment.
