...
weight: 80
description: Migrating from Gloo Gateway 0.x to Gloo Gateway 1.0.0 
---
...
- [Gloo Gateway Enterprise]({{< versioned_link_path fromRoot="/operations/upgrading/v1.0/#gloo-edge-enterprise" >}})
...
We have officially released Gloo Gateway 1.0.0! This major version bump comes with a number of breaking changes
that you will have to keep in mind as you upgrade from Gloo Gateway 0.x to 1.0.0+. While the easiest upgrade 
process is to just start with a totally fresh installation of Gloo Gateway, many users are running Gloo Gateway in 
production on older versions; this guide is provided for users like them who would like to update to our 
latest and greatest Gloo Gateway without any downtime.
...
old behavior. https://github.com/solo-io/gloo/pull/1341
   - By default, Gloo Gateway now only propagates config from Virtual Services in the same namespace as the referencing Gateway
...
- Move the Helm values field "installConfig.installationId" to "global.glooInstallationId" in order to better facilitate usage of the Gloo Gateway sub-chart in Gloo Gateway Enterprise (https://github.com/solo-io/gloo/issues/1635)
- Remove some deprecated APIs:
    - Remove CorsPolicy from Gateway and Gloo Gateway VirtualHost, prefer setting CorsPolicy on VirtualHostPlugins.
...
- Rename all instances of plugins to options in Gloo Gateway's API. This prevents confusion as Gloo Gateway "plugins" is really an implementation detail for devs, and Gloo Gateway does not currently support dynamically-loaded plugins (aside from ExtAuth plugins).
- Update ExtAuth secret API to use strongly-typed configuration. OAuth and ApiKey secrets are no longer configured in the opaque extensions block, the same configuration lives at the top level in the api_key and oauth blocks. (https://github.com/solo-io/gloo/issues/1171) 
...
- This release changes the names of Gloo Gateway's ClusterRoles. By default, cluster-scoped roles will have the namespace of the associated Gloo Gateway installation appended to their name (i.e., the cluster-scoped role "gloo-resource-reader" created along with a Gloo Gateway installation to the gloo-system namespace will now become "gloo-resource-reader-gloo-system"). This may cause a problem during upgrades from Gloo Gateway less than 0.21.0 to Gloo Gateway greater than or equal to 0.21.0 for both open-source and enterprise Gloo Gateway, as the role ref in a ClusterRoleBinding is immutable. To resolve this, you can delete the existing ClusterRoleBindings with "kubectl delete clusterrolebinding -l app=gloo" and they will be recreated correctly by the rest of the upgrade process. (https://github.com/solo-io/gloo/issues/1459)
...
- Add support for multiple matchers on a Gateway/Gloo Gateway Route to reduce user duplication in yaml/user code. Also allows users to omit matchers on routes (the / prefix matcher will be used) to allow all requests to match the route. (https://github.com/solo-io/gloo/issues/1171)
...
- Remove some deprecated APIs:
    ...
    - Extauth's VhostExtension and RouteExtension, among other minor removals. Prefer configuring Gloo Gateway Enterprise ExtAuth using AuthConfig Custom Resources, and configure Virtual Services via ExtAuthExtension to either reference these AuthConfigs or reference your own custom auth implementation using CustomAuth. (https://github.com/solo-io/gloo/issues/1171)
...
[here]({{< versioned_link_path fromRoot="/operations/upgrading/faq" >}})), both of which may also contain useful tips for performing Gloo Gateway upgrades in general.
{{% /notice %}}

In this section, we will walk through the process of upgrading a very simple Gloo Gateway installation (running in
...
{{% notice note %}}
This guide will assume that you are running Gloo Gateway in the gloo-system namespace.
{{% /notice %}}
...
Now we start the upgrade process. Before we begin, we may want to dump the current Gloo Gateway state to a file.
...
Congratulations! You've just performed a major Gloo Gateway upgrade without incurring any downtime.
...
will also delete the resources from your new 1.0.0 installation, and Gloo Gateway may experience downtime.
{{% /notice %}}
...
Removing Gloo Gateway system components from namespace gloo-system...

Gloo Gateway was successfully uninstalled.
```
...
There are several points to consider about Helm compatibility if you are upgrading Gloo Gateway (open source or enterprise)
across the 0.x to 1.x boundary:

* Helm 2 **IS NOT** compatible with the Open Source Gloo Gateway chart in Gloo Gateway versions v1.2.0 through v1.2.2.
* However, Helm 2 **IS** compatible with all stable versions of the Gloo Gateway Enterprise chart.
* `glooctl` prior to v1.2.0 cannot be used to install open source Gloo Gateway v1.2.0 and later or Gloo Gateway Enterprise v1.2.0 and later.

## Gloo Gateway Enterprise

Gloo Gateway Enterprise also had a 1.x.y major release! If you are a Gloo Gateway Enterprise user, please consult both this section
...
You may notice that Gloo Gateway Enterprise has released directly to 1.2.0 from 0.x. This is because we keep the major/minor
versions of Open Source Gloo Gateway and Gloo Gateway Enterprise in sync, and Open Source Gloo Gateway has progressed to 1.2.x. 

### Enterprise Breaking Changes

Since Enterprise is a superset of Open Source Gloo Gateway, you should also consult the comprehensive list of
[all breaking changes]({{< versioned_link_path fromRoot="/operations/upgrading/v1.0/#all-breaking-changes" >}}) within Open Source
Gloo Gateway if you are upgrading Enterprise from 0.x to >=1.2.0. 

In addition to the open source breaking changes, Gloo Gateway Enterprise 1.2.0 also includes the following breaking changes:

- Remove some deprecated APIs:
    - `weighed_destination_plugins` on `WeightedDestinations`, prefer `weighted_destination_options`
    - `gateway_proxy_name` on `Gateway`, prefer `proxy_names`
    - `role_arns` on UpstreamSpec, prefer `role_arn`
    - Extauth's `VhostExtension` and `RouteExtension`, among other minor removals. Prefer configuring Gloo Gateway Enterprise ExtAuth 
