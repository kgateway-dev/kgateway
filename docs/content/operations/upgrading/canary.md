...
weight: 30
description: Upgrading Gloo Gateway in a canary model
---

You can upgrade your Gloo Gateway deployments by following a canary model. In the canary model, you make two different `gloo` deployments in your data plane, one that runs your current version and one for the target version to upgrade to. Then, you check that the deployment at the target version handles traffic as you expect before upgrading to run the target version. This approach helps you reduce potential downtime for production upgrades.

## Before you begin

1. Install Gloo Gateway Open Source or Enterprise **version 1.9.0 or later**, or Federation **version 1.13.0 or later**.
...
3. If you have Gloo Gateway Enterprise or Federation, set your license key as an environment variable. To request a license, [contact Sales](https://www.solo.io/company/contact/).
   ```
   export GLOO_LICENSE=<license>
   ```
4. Set the target version that you want to upgrade to as an environment variable. To find available versions, check the [changelog]({{< versioned_link_path fromRoot="/reference/changelog/" >}}). The following commands include the latest versions for each of the following Gloo Gateway installation options.
   {{< tabs >}} 
...
6. Check the [upgrade notice for the minor version]({{< versioned_link_path fromRoot="/operations/upgrading/" >}}) and the [changelogs for the patch version]({{< versioned_link_path fromRoot="/reference/changelog/" >}}) that you want to upgrade to. In particular, review the following changes:
   * **CRD changes**: Each patch version might add custom resource definitions (CRDs), update existing CRDs, or remove outdated CRDs. When you perform a canary upgrade, the existing Gloo Gateway CRDs are not updated to the newer version automatically. You must manually apply the new CRDs first. The Gloo Gateway CRDs are designed to be backward compatible, so the new CRDs should not impact the performance of your older installation. However, if after evaluating the newer installation you decide to continue to use the older installation, you can easily remove any added CRDs by referring to the upgrade notices for the CRD names and running `kubectl delete crd <CRD>`. Then, to re-apply older versions of CRDs, you can run `helm pull gloo/gloo --version <older_version> --untar` and `kubectl apply -f gloo/crds`.
...
## Upgrade Gloo Gateway Open Source or Enterprise in a canary model {#canary-upgrade}

1. Update and pull the Gloo Gateway Helm chart for the target version.
   {{< tabs >}} 
{{< tab name="Open Source" codelang="shell" >}}
...
{{< /tab >}}
   {{< /tabs >}}
3. Install the target version of Gloo Gateway in the new namespace in your cluster.
   {{< tabs >}} 
{{< tab name="Open Source" codelang="shell" >}}
...
{{< /tab >}}
   {{< /tabs >}}
4. Verify that your current and target versions of Gloo Gateway are running.
   ```shell
...
6. Test your routes and monitor the metrics of the newer version.
    ...
    glooctl check
    ```
7. [Uninstall]({{< versioned_link_path fromRoot="/reference/cli/glooctl_uninstall/" >}}) the older version of Gloo Gateway so that your cluster uses the newer version going forward.
   ```shell
   gloooctl uninstall -n gloo-system
   ```

## Upgrade Gloo Gateway Federation in a canary model {#canary-upgrade-fed}

You can upgrade Gloo Gateway Federation in a canary model in version 1.13 or later. In [Gloo Gateway Federation]({{< versioned_link_path fromRoot="/guides/gloo_federation/" >}}), you have a management cluster that runs Gloo Gateway Federation (and optionally, other Enterprise components). Then you register remote clusters with Gloo Gateway Enterprise.

In the canary upgrade model, you start with an older version of Gloo Gateway Federation in your management cluster that manages one or more remote Gloo Gateway instances. To test a newer target version, you install the target version of Gloo Gateway Federation in a new namespace on the management cluster. Then, you install one or more Gloo Gateway instances at the target version in remote test clusters, register the new clusters, and check your setup. Finally, you deregister the remote clusters from the older version of Gloo Gateway Federation and uninstall the older Gloo Gateway Federation version from the management cluster.
...
2. Apply the new Gloo Fed CRDs from the target version Helm chart to your management cluster.
   ...
   ```
3. Install the target version of Gloo Gateway Federation in the new namespace in your management cluster.
   ```
   ...
   ```
4. Verify that your current and target versions of Gloo Gateway Federation are running. 
   ```shell
   kubectl get all -n gloo-system-$OLD_VERSION
   kubectl get all -n gloo-system-$TARGET_VERSION
   ```
5. [Install and register Gloo Gateway Enterprise]({{< versioned_link_path fromRoot="/guides/gloo_federation/cluster_registration/" >}}) on one of your remote clusters. Make sure that the version of Gloo Gateway Enterprise matches the version of Gloo Gateway Federation that you installed earlier.
6. In your management cluster, [create or modify any federated custom resources]({{< versioned_link_path fromRoot="/guides/gloo_federation/federated_configuration/" >}}) needed to test your target version of Gloo Federation/Edge. Because Gloo Gateway Federation scans all federated resources on the management cluster, you might want to reuse the existing federated resources in the `gloo-system-$OLD_VERSION` namespace. Include any changes or new capabilities that you noticed in the [upgrade notice]({{< versioned_link_path fromRoot="/operations/upgrading/" >}}) and the [changelogs]({{< versioned_link_path fromRoot="/reference/changelog/" >}}) for the target version. 
   ...
   * Creates a shared `gloo-fed` namespace to show that both the old and new Gloo Gateway Federation instances can watch for changes to a resource in the same namespace.
...
8. Test your routes and monitor the metrics of the target version in your remote test cluster.
    ...
    glooctl check
    ```
9. Shift traffic to the target version of Gloo Gateway Federation by uninstalling the older version.
   1. [Deregister]({{< versioned_link_path fromRoot="/reference/cli/glooctl_cluster_deregister/" >}}) your other remote clusters that still use the old version of Gloo Gateway Federation.
   2. [Uninstall]({{< versioned_link_path fromRoot="/reference/cli/glooctl_uninstall/" >}}) the older version of Gloo Gateway Federation so that your management cluster uses the newer version going forward.
      ```shell
...
As you test your environment with the new version of Gloo Gateway, you might need to roll back to the previous version. You can follow a canary model for the rollback.

1. Set the previous version that you want to roll back to as an environment variable.
   ```shell
   export ROLLBACK_VERSION=<version>
   ```
2. If you already removed the previous version of Gloo from your cluster, re-install Gloo Gateway.
   1. Update and pull the Gloo Gateway Helm chart for the target version. The Helm charts vary depending on the Gloo Gateway installation option.
      {{< tabs >}} 
{{< tab name="Open Source" codelang="shell" >}}
helm repo update
...
{{< /tab >}} 
      {{< /tabs >}}
   2. Install the rollback version of Gloo Gateway in the new namespace in your cluster.
      {{< tabs >}} 
{{< tab name="Open Source" codelang="shell" >}}
...
2. **Gloo Gateway Federation**: Shift traffic from the newer target version to the rollback version of Gloo Gateway Federation.
   1. [Deregister]({{< versioned_link_path fromRoot="/reference/cli/glooctl_cluster_deregister/" >}}) your remote clusters that still use the newer target version of Gloo Gateway Federation.
   2. [Install and register Gloo Gateway Enterprise]({{< versioned_link_path fromRoot="/guides/gloo_federation/cluster_registration/" >}}) for the rollback version's Gloo Gateway Federation on each remote cluster.
   3. Optionally delete the newer target version namespace. The deregister command does not clean up the namespace and custom resources in the old version.
      ...
      ```
3. [Uninstall]({{< versioned_link_path fromRoot="/reference/cli/glooctl_uninstall/" >}}) the newer target version of Gloo Gateway from your cluster.
    ```shell
    gloooctl uninstall -n gloo-system-$TARGET_VERSION
    ```
4.  Apply the Gloo CRDs from the rollback version Helm chart to your cluster. In Gloo Gateway Federation, this cluster is the local management cluster.
   {{< tabs >}} 
{{< tab name="Open Source" codelang="shell" >}}
kubectl apply -f gloo/crds
{{< /tab >}}
