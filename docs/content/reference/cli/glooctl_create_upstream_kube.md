...
Kubernetes Upstreams represent a collection of endpoints for Services registered with Kubernetes. Typically, Gloo Edge will automatically discover these upstreams, meaning you don't have to create them. However, if upstream discovery in Gloo Edge is disabled, or RBAC permissions have not been granted to Gloo Edge to read from the registry, Kubernetes services can be added to Gloo Edge manually via the CLI.
