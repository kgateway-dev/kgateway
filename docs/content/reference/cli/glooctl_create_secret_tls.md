...
Create a secret with the given name. The format of the secret data is: `{"tls" : [tls object]}`. Note that the annotation `resource_kind: '*v1.Secret'` is added in order for Gloo Edge to find this secret. If you're creating a secret through another means, you'll need to add that annotation manually.
