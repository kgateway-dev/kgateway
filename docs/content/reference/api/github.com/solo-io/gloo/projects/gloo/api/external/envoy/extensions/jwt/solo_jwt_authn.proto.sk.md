...
### Package: `solo.io.envoy.config.filter.http.solo_jwt_authn.v2` 
#### Types:
...
```yaml
"requirement": string
"claimsToHeaders": map<string, .solo.io.envoy.config.filter.http.solo_jwt_authn.v2.SoloJwtAuthnPerRoute.ClaimToHeaders>
...
```yaml
"claims": []solo.io.envoy.config.filter.http.solo_jwt_authn.v2.SoloJwtAuthnPerRoute.ClaimToHeader

```

| Field | Type | Description |
| ----- | ---- | ----------- | 
| `claims` | [[]solo.io.envoy.config.filter.http.solo_jwt_authn.v2.SoloJwtAuthnPerRoute.ClaimToHeader](../solo_jwt_authn.proto.sk/#claimtoheader) |  |
