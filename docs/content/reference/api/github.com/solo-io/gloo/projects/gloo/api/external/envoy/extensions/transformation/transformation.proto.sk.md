...
| `REPLACE_ALL` | Replace all matches of the regex in the source with the replacement_text. Note: replacement_text must be set for this mode. Note: configuration will fail if subgroup is set. Note: restrictions on the regex are different for this mode. See the regex field for more details. |
