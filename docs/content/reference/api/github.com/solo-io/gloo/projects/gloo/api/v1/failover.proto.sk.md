...
| `metadata` | `map<string, .google.protobuf.Struct>` | Additional metadata to put on this endpoint, This metadata can be used in upstream HTTP filters or other specific envoy configurations. 2 of these keys are spceified by gloo and will be ignored if set: - "envoy.transport_socket_match" - "io.solo.health_checkers.advanced_http". |
