...
These values are similar to those seen in COMMON_DURATIION 
introduced in envoy 1.31.
It can also emit 3 additional metadata fields: 
...
These dynamic metadata fields can be used to augment the existing upstream options for Access logging
...
| ----- | ---- | ----------- | 
| `request` | [.envoy.config.filter.http.proxylatency.v2.ProxyLatency.Measurement](../proxylatency.proto.sk/#measurement) | How to measure the request. Affects the output stats. Does not affect metdata. |
...
| `emitDynamicMetadata` | [.google.protobuf.BoolValue](https://developers.google.com/protocol-buffers/docs/reference/csharp/class/google/protobuf/well-known-types/bool-value) | Should we emit request timing to dynamic metadata. If enabled it defaults to true. |
