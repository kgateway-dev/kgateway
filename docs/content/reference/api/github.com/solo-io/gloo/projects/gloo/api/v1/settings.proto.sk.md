...
| `namedExtauth` | `map<string, .enterprise.gloo.solo.io.Settings>` | Enterprise-only: External auth related settings for additional auth servers This should only be used in the case where separate servers are needed to authorize separate routes. Even with multiple auth servers configured in Settings, only 1 will ever be configured and executed on a route. The name of the auth server (ie the key in the map) will be used to apply the configuration on the route. |
