...
| `fallbackToFirstFunction` | [.google.protobuf.BoolValue](https://developers.google.com/protocol-buffers/docs/reference/csharp/class/google/protobuf/well-known-types/bool-value) | Sets the unsafe behavior where a route can specify a lambda upstream but not set the function to target. If will use the first function. If discovery is enabled the first function is the first function name alphabetically from the last discovery run. Defaults to false. |
