...
| `useV3TransportApi` | `bool` | If set to `true`, Gloo Edge will configure Envoy to communicate with the auth server using `V3` of the `ext_authz` API. Defaults to `false`, which will use the default protocol version, which currently is `V2`. For more info, see the `transport_api_version` field [here](https://www.envoyproxy.io/docs/envoy/latest/api-v3/extensions/filters/http/ext_authz/v3/ext_authz.proto#extensions-filters-http-ext-authz-v3-extauthz). |
