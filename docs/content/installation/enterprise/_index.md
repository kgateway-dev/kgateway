...
| gloo.gloo.disableLeaderElection | bool | Set this value to `false` when you configure multiple replicas of the `gloo` deployment in your installation. This way, Gloo Edge elects a leader from the replicas, with the other replicas ready to become leader if needed for higher availability. If you just have 1 replica of Gloo Edge, you can set this value to `true`.|
