...
| gloo.gloo.disableLeaderElection | bool | Leave this field set to the default value of `false` for multiple replicas of the `gloo` deployment in your installation. This way, Gloo Edge elects a leader from the replicas, with the other replicas ready to become leader if needed for higher availability. If you want to run only one replica of `gloo`, you can set this value to `true`.|
