...
old behavior. https://github.com/solo-io/gloo/pull/1341
   - By default (i.e., `virtual_services` and `virtual_service_selector` are omitted on the Gateway), Gloo now only
propagates config from Virtual Services in the same namespace as the referencing Gateway (compare in
[0.21.1](https://github.com/solo-io/gloo/blob/v0.21.1/projects/gateway/api/v2/gateway.proto#L84-L91) to
...
in Gloo 1.2.0.
   ...
   - All instances of `...plugins` in our API (e.g., `virtualHostPlugins`, `routePlugins`) has been renamed to `options`.
