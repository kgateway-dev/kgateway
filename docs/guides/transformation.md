...
- RustFormation: This is written in rust using Envoy Dynamic Module. The module is bundle into the envoy_wrapper image together with the Envoy binary. It's loaded at run time when Envoy starts up.
...
Before v2.2, the Classic Transformation is the default implementation. With the release of v2.2, we switched to Rustformation as the default implementation. However, there are some differences between the two implementations because of the underlying Inja templating library used.
...
| Body buffering | The C++ Transformation filter buffer the data in side it's own structure and do it's own buffer limit detection using Envoy's decoder buffer limit setting | Rely on Envoy to do the buffering before we process the entire body | |
